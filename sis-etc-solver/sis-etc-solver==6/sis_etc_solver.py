...
def control_parameter_solver_gp(p, rc, W, M, d):
    ...
    gp_fc = 1
    for i in range(n):
        gp_fc += bark[i][i] * 10000000 / tildek[i][i]
        for j in range(n):
            if B[i][j] != 0:
...
def plot_data(p, K, L, sigma, eta, M, d_table, choice):
        ...
        # # In the case of using feedback controller
            ...
            for i in range(n):
                ...
                # # choice 3 is the case of event-triggered controller
                    ...
                    xk[i] = x[k][i]
                    event[k + 1][i] = 1
            x[k + 1] = x[k] + h * (-(Dn * 1.2 + Kn.dot(np.diag(xk))).dot(x[k]) + (
    ...
    cm = plt.cm.get_cmap('cool', n)
    for i in range(n):
        ax.plot(x.T[i], lw=2, color=cm(i), label=node_labels[i])
        # ax.plot(x.T[i], lw=2, label=node_labels[i])
    ...
    #           fontsize=19.5, title=r'airport code', ncol=3).get_title().set_fontsize(30)
    plt.tight_layout()
    plt.grid()
    if choice == 1:
